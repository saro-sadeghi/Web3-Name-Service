...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) ERC721 ("Adakpro Name Service", "ADK") payable {
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    // Ensure the length is greater than 0
    require(len > 0);
    // Check if the length is 3
    if (len == 3) {
      return 1 * 10**15; //0.001 MATIC = 1 000 000 000 000 000
    }
  ...
  //function for buying domain
  function register(string calldata name) public payable{
    if (domains[name] != address(0)) revert AlreadyRegistered();
    if (!valid(name)) revert InvalidName(name);
    //cheack 
    require(domains[name] == address(0), "The domains already have owner");
    //get the price
    ...
    string memory json = Base64.encode(
        abi.encodePacked(
            '{'
                '"name": "', _name,'", '
                '"description": "A domain on the Neld name service", '
  ...
  //get the owner of domain by domain name
  ...
  //only owner 
  function registerFree(string calldata name) public onlyOwner {
    if (domains[name] != address(0)) revert AlreadyRegistered();
    if (!valid(name)) revert InvalidName(name);
  ...
  function transferEther(string memory name, uint amount) public payable {
        address recipient = domains[name];
        require(msg.value == amount, "Incorrect amount");
        payable(recipient).transfer(msg.value);

    }
    //owner can transfer domain
    ...
    require(msg.sender == domains[name], "Only the current owner can transfer ownership");
